...
(window as any).__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = [
			"#dsh-quota-status{position:fixed;right:18px;bottom:18px;z-index:900;display:flex;flex-direction:column;align-items:flex-end;pointer-events:auto;color:var(--dsw-alias-label-primary,#1b1b1c);font-family:var(--dsw-font-family,-apple-system,BlinkMacSystemFont,\"Segoe UI\",\"PingFang SC\",\"Microsoft YaHei\",sans-serif);font-size:13px;line-height:1.45}",
			"#dsh-quota-capsule{display:inline-flex;align-items:center;gap:8px;height:32px;padding:0 12px 0 14px;border:1px solid var(--dsw-alias-border-l2,rgba(0,0,0,.1));border-radius:18px;background:var(--dsw-alias-bg-layer-2,#fff);box-shadow:var(--dsw-shadow-lv2,0 4px 12px rgba(15,17,21,.02),0 2px 8px rgba(15,17,21,.04));cursor:pointer;font:inherit;transition:background-color 120ms ease}",
			...
			"#dsh-quota-card{width:308px;max-width:calc(100vw - 36px);margin-top:8px;padding:14px 16px 15px;border:1px solid var(--dsw-alias-border-l2,rgba(0,0,0,.1));border-radius:16px;background:var(--dsw-alias-bg-layer-2,#fff);box-shadow:var(--dsw-shadow-lv3,0 8px 26px rgba(15,17,21,.1),0 2px 6px rgba(15,17,21,.06));box-sizing:border-box}",
			"#dsh-quota-card .dsh-quota-header{display:flex;align-items:center;justify-content:space-between;min-height:24px;margin-bottom:12px}",
			...
			"#dsh-quota-card .dsh-window{margin-top:8px}",
			"#dsh-quota-card .dsh-window-head{display:flex;align-items:baseline;justify-content:space-between;gap:10px;color:var(--dsw-alias-label-secondary,#61666b);font-size:12px;line-height:18px}",
			"#dsh-quota-card .dsh-window-value{font-variant-numeric:tabular-nums;font-weight:600}",
			"#dsh-quota-card .dsh-progress{position:relative;width:100%;height:5px;margin-top:5px;overflow:hidden;border-radius:999px;background:var(--dsw-alias-bg-overlay,#ebeef2)}",
			...
			"#dsh-quota-card .dsh-window-caption{margin-top:5px;color:var(--dsw-alias-label-tertiary,#818590);font-size:11px;line-height:16px;font-variant-numeric:tabular-nums}",
			"#dsh-quota-card .dsh-quota-divider{height:1px;margin:12px 0;background:var(--dsw-alias-border-l1,rgba(0,0,0,.06))}",
		...
		function apply(ctx) {
			...
			function QuotaStatus(props) {
				...
				if (!expanded) {
					...
					return React.createElement("div", { id: "dsh-quota-status" },
